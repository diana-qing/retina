Filter: !tls, Datatypes: ["DnsTransaction"], Callback: "not_tls_cb"
...
filter_subtree: deliver: Deliver {
    id: 0,
    as_str: "not_tls_cb(DnsTransaction)",
    must_deliver: false,
}
...
pub fn filter() -> retina_core::filter::FilterFactory<TrackedWrapper> {
    ...
    fn session_filter(
        ...
        if !match &session.data {
            retina_core::protocols::stream::SessionData::Tls(tls) => true,
            _ => false,
        } {
            if let Some(s) = DnsTransaction::from_session(session) {
                not_tls_cb(s);
            }
        }
        result
