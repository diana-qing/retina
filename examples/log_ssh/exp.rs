Filter: ssh.software_version_ctos ~ '^OpenSSH_[0-9]+\.[0-9].*$', Datatypes: ["SshHandshake"], Callback: "ssh_cb"
Filter: ssh, Datatypes: ["SshHandshake"], Callback: "log_ssh"
...
Tree Proto
,`- 0: ethernet
   `- 1: ssh -- A: Actions { data: ActionData[SessionFilter, SessionDeliver], terminal_actions: ActionData[] }

Tree S
,`- 0: ethernet
   `- 1: ssh D: ( log_ssh(SshHandshake), )
      `- 2: ssh.software_version_ctos matches ^OpenSSH_[0-9]+\.[0-9].*$ D: ( ssh_cb(SshHandshake), )
...
impl clap::CommandFactory for Args {
    fn into_app<'b>() -> clap::Command<'b> {
        let __clap_app = clap::Command::new("log_ssh");
...
impl Trackable for TrackedWrapper {
    ...
    fn parsers() -> retina_core::protocols::stream::ParserRegistry {
        retina_core::protocols::stream::ParserRegistry::from_strings(
            <[_]>::into_vec(#[rustc_box] ::alloc::boxed::Box::new(["ssh"])),
        )
    }
}
#[allow(missing_copy_implementations)]
...
impl ::lazy_static::__Deref for RE0 {
    ...
    fn deref(&self) -> &regex::Regex {
        ...
        fn __static_ref_initialize() -> regex::Regex {
            regex::Regex::new("^OpenSSH_[0-9]+\\.[0-9].*$").unwrap()
        }
        #[inline(always)]
...
pub fn filter() -> retina_core::filter::FilterFactory<TrackedWrapper> {
    ...
    fn session_filter(
        ...
        let mut result = retina_core::filter::Actions::new();
        if let retina_core::protocols::stream::SessionData::Ssh(ssh) = &session.data {
            if RE0.is_match(&ssh.software_version_ctos()[..]) {
                if let Some(s) = SshHandshake::from_session(session) {
                    ssh_cb(s);
                }
            }
            if let Some(s) = SshHandshake::from_session(session) {
